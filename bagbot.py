...
class BittensorUtility():
    ...
    def _load_trade_counts(self):
        """Charge le compteur de trades depuis le fichier JSON."""
        try:
            with open(self.TRADE_COUNTS_FILE, 'r') as f:
                raw = json.load(f)
                self.trade_counts = {int(k): v for k, v in raw.items()}
            logger.info(f"Compteur de trades chargé depuis {self.TRADE_COUNTS_FILE}")
        except FileNotFoundError:
            logger.info("Aucun fichier trade_counts.json trouvé, démarrage à zéro")
        except Exception as e:
            logger.error(f"Erreur chargement trade_counts: {e}")

    def _save_trade_counts(self):
        """Sauvegarde le compteur de trades dans le fichier JSON."""
        try:
            with open(self.TRADE_COUNTS_FILE, 'w') as f:
    ...
    async def setup(self):
        ...
        logger.info('Started')
        self.sendNotification("🚀 BagBot démarré et connecté au réseau Bittensor")
    ...
    def _telegram_send(self, text: str) -> bool:
        """Envoi bas-niveau vers l'API Telegram. Retourne True si succès."""
        token   = getattr(bagbot_settings, 'TELEGRAM_TOKEN',   '')
        chat_id = getattr(bagbot_settings, 'TELEGRAM_CHAT_ID', '')
        if not token or not chat_id:
            return False  # Telegram non configuré, on passe silencieusement
        try:
            resp = requests.post(
            ...
            logger.error(f"[Telegram] Échec envoi: {e}")
            return False
 
    def sendNotification(self, msg: str):
        """Notification générique (erreurs, alertes système)."""
        logger.info(msg)
        ts = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self._telegram_send(f"ℹ️ *BagBot*\n{msg}\n🕒 {ts}")
 
    def _notify_buy_success(self, trade: dict, result):
        """Trade BUY exécuté avec succès."""
        netuid  = trade['netuid']
        amount  = float(trade['tao_amount'])
        slip    = float(trade['calculated_slippage'])
        thresh  = float(trade['buy_threshold'])
        price   = self.stats.get(netuid, {}).get('price', '?')
        name    = self.stats.get(netuid, {}).get('name', '')
        ts      = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        msg = (
            f"🟢 *BUY exécuté* — SN{netuid} {name}\n"
            f"├ Montant : `{amount:.4f} TAO`\n"
            f"├ Prix    : `{price}`\n"
            f"├ Seuil   : `{thresh}`\n"
            f"├ Slippage: `{slip:.4f}%`\n"
            f"└ 🕒 {ts}"
        )
        logger.info(f"[Telegram] BUY SN{netuid}")
        self._telegram_send(msg)
 
    def _notify_buy_failed(self, trade: dict, reason: str):
        """Trade BUY échoué."""
        netuid = trade['netuid']
        amount = float(trade['tao_amount'])
        ts     = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        msg = (
            f"⚠️ *BUY échoué* — SN{netuid}\n"
            f"├ Montant : `{amount:.4f} TAO`\n"
            f"├ Raison  : `{reason}`\n"
            f"└ 🕒 {ts}"
        )
        logger.warning(f"[Telegram] BUY échoué SN{netuid}: {reason}")
        self._telegram_send(msg)
 
    def _notify_sell_success(self, trade: dict, result):
        """Trade SELL exécuté avec succès."""
        netuid   = trade['netuid']
        alpha    = float(trade['alpha_amount'])
        approx   = float(trade['approx_tao'])
        slip     = float(trade['calculated_slippage'])
        thresh   = float(trade['sell_threshold'])
        price    = self.stats.get(netuid, {}).get('price', '?')
        name     = self.stats.get(netuid, {}).get('name', '')
        ts       = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        msg = (
            f"🔴 *SELL exécuté* — SN{netuid} {name}\n"
            f"├ Alpha   : `{alpha:.4f}`\n"
            f"├ ~TAO    : `{approx:.4f} TAO`\n"
            f"├ Prix    : `{price}`\n"
            f"├ Seuil   : `{thresh}`\n"
        ...
        logger.info(f"[Telegram] SELL SN{netuid}")
        self._telegram_send(msg)
 
    def _notify_sell_failed(self, trade: dict, reason: str):
        """Trade SELL échoué."""
        netuid = trade['netuid']
        approx = float(trade.get('approx_tao', 0))
        ts     = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        msg = (
            f"⚠️ *SELL échoué* — SN{netuid}\n"
            f"├ ~TAO   : `{approx:.4f} TAO`\n"
            f"├ Raison : `{reason}`\n"
            f"└ 🕒 {ts}"
        )
        logger.warning(f"[Telegram] SELL échoué SN{netuid}: {reason}")
        self._telegram_send(msg)
    ...
    async def run(self):
        ...
        while True:
            ...
            try:
                ...
                if "await wasn't used with future" in str(e) or "Future" in str(e):
                    logger.warning(f'Asyncio future error: {e}, reconnecting subtensor...')
                    self.sendNotification(f"⚠️ Erreur asyncio transitoire, reconnexion en cours...")
                    try:
                        await self.sub.close()
                    except:
                        pass
    ...
    async def do_available_trades(self, subnet_netuid):
        ...
        if buyTrade:
            try:
                ...
                if stake_result is True or stake_result.__dict__.get('success') is True:
                    ...
                    self._notify_buy_success(buyTrade, stake_result)
                    netuid = buyTrade['netuid']
                ...
                self._notify_buy_failed(buyTrade, "Timeout 45s")
            except Exception as e:
                print(f'ERROR staking')
                logger.error(traceback.format_exc())
                logger.error(f"Failed to stake on subnet {buyTrade['netuid']}: {e}")
                self._notify_buy_failed(buyTrade, str(e))
        ...
        if sellTrade:
            try:
                ...
                if unstake_result is True or (hasattr(unstake_result, '__dict__') and unstake_result.__dict__.get('success') is True):
                    ...
                    self._notify_sell_success(sellTrade, unstake_result)
                    netuid = sellTrade['netuid']
                ...
                print(msg)
                logger.error(msg)
                self._notify_sell_failed(sellTrade, "Timeout 60s")
                self.sub = await my_async_subtensor("finney")
                ...
                self._notify_sell_failed(sellTrade, f"CancelledError: {e}")
                self.sub = await my_async_subtensor("finney")
            except Exception as e:
                print(f'ERROR unstaking')
                logger.error(traceback.format_exc())
                logger.error(f"Failed to unstake from subnet {subnet_netuid}: {e}")
                self._notify_sell_failed(sellTrade, str(e))
                raise
